...
class LoginForm(forms.Form):
    username = forms.CharField(label='这是用户名', widget=forms.TextInput(attrs={'class':'fom-control','placeholder':'没输入用户名'}))
    password = forms.CharField(label='这是密码', widget=forms.TextInput(attrs={'class': 'fom-control','placeholder':'没输入用户名' }))
    ...
    def clean(self):
        ...
        if user is None:
            raise forms.ValidationError('用户名或密码不行噢')
        else:
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.user.is_authenticated:
            ...
            raise forms.ValidationError('用户没登陆')
        return self.cleaned_data
